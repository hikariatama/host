...
Проверка фильтра обсценной лексики"""
		...
		if raw_text != censtext:
			...
			await message.edit(self.strings['cens_no'] + (censtext if not message.is_reply else ''))
	...
	async def censlistcmd(self, message):
		...
		censlist = [f'<code>{x}</code>{"*" if ids[x]==2 else ""}' for x in ids if isinstance(x, int) and ids[x] > 0]
		if not censlist:
			return await message.edit(self.strings['censlist_empty'])
...
* запускает на все сообщения в текущем чате (по умолчанию на свои)"""
		...
		if args == '*':
			chat = message.chat
			if not isinstance(chat, Channel) or not chat.admin_rights or not chat.admin_rights.delete_messages:
		...
		if not args:
			...
			try: id = (await self.client.get_entity(args)).id
			except: return await message.edit(self.strings['censon_invalid'])
...
Команда без аргументов выводит список кастомных исключений"""
		...
		if not args:
			exc = self.db.get(self.name, 'exc', [])
			if not exc: return await message.edit(self.strings['censx_empty'])
			return await message.edit(self.strings['censx_cmd'].replace('%list%', ' '.join([f'<code>{x}</code>' for x in exc])))
	...
	async def watcher(self, event):
		...
		if not flag or (flag == 1 and event.sender_id != self.me_id): return
		exc = self.db.get(self.name, 'exc', [])
		censtext = OCR.filterText(event.raw_text, exc=exc)
		if censtext == event.raw_text: return
		message = event
		...
		if flag == 2 and event.sender_id != self.me_id: return
