...
class NSFWMod(loader.Module):
    """Кидает рандомную картинку NSFW содержания с scroller"""
    strings = {
        "name": "NSFW",
    ...
    async def nsfwcmd(self, message):
        ...
        ans = requests.get(
            ...
            json={
                ...
                + str(quantity)
                + " iterator: $iterator filter: $filter ) { iterator items { __typename url title subredditTitle subredditUrl redditPath isNsfw albumUrl isFavorite mediaSources { url width height isOptimized } } } } } ",
                "variables": {"filter": None, "url": subreddit},
                "authorization": None,
            },
        ).json()
