...
class SeeChatMod(loader.Module):
    ...
    async def gseecmd(self, message):
        ...
        try:
            ...
            return await message.edit("<b>file is empty.</b>")
    ...
    async def delseecmd(self, message):
        ...
        if args == "all":
            ...
            await message.edit(
                "<b>all PM chats file has been successfully deleted.</b>"
            )
        else:
            try:
                user = await message.client.get_entity(int(args))
                await message.edit(
                    f"<b>the chat file has been deleted with: <code>{user.first_name}</code></b>"
                )
                os.remove(f"SeeChat/{args}.txt")
            except:
                return await message.edit("<b>file can't be deleted.</b>")
    ...
    async def excseecmd(self, message):
        ...
        if args == "clear":
            ...
            return await message.edit(
                "<b>the exclusion list was cleared successfully.</b>"
            )
        try:
            user = await message.client.get_entity(int(args))
            if str(user.id) not in exception:
                ...
                await message.edit(
                    f"<b>{user.first_name}, has been added to the list of exclusions.</b>"
                )
                os.remove(f"SeeChat/{user.id}.txt")
            else:
                exception.remove(str(user.id))
                await message.edit(
                    f"<b>{user.first_name}, has been removed from the list of exclusions.</b>"
                )
            self.db.set("SeeChat", "exception", exception)
        except:
            return await message.edit(
                "<b>failed to remove user from the list of exclusions</b>"
            )
    ...
    async def exclistcmd(self, message):
        ...
        try:
            ...
            return await message.edit("<b>the list of users is empty.</b>")
        ...
        if (
            ...
            if message.photo:
                ...
                await message.client.send_message(
                    chat.id,
                    f"<b>picture from</b> <a href='tg://user?id={user.id}'>{user.first_name}</a>:",
                )
            ...
            if (
                ...
                try:
                    ...
                    await message.client.send_message(
                        chat.id,
                        f"<b>Video from</b> <a href='tg://user?id={user.id}'>{user.first_name}</a>:",
                    )
