...
class MinusMod(loader.Module):
    ...
    async def client_ready(self, client, db):
        global overlays
        overlays = {
            'pablo': (await utils.run_sync(requests.get, "https://x0.at/9z_9.mp3")).content,
            'kids': (await utils.run_sync(requests.get, "https://x0.at/4bfh.mp3")).content,
            'camry': (await utils.run_sync(requests.get, "https://x0.at/W8ui.mp3")).content,
            'gta': (await utils.run_sync(requests.get, "https://x0.at/KCkV.mp3")).content,
            'amogus': (await utils.run_sync(requests.get, "https://x0.at/wPms.mp3")).content,
            'chill': (await utils.run_sync(requests.get, "https://x0.at/JBs4.mp3")).content
        }
