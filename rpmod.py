                ...
                if not key_rp or not key_rp.strip():
                    ...
                    if not value_rp or not value_rp.strip():
                        ...
                        if int(len(key_len)) > 1:
                            ...
                            if key_rp == 'all':
                                await message.edit('<b>Использовать \'<code>all</code>\' в качестве названия команды запрещено!</b>')
                            else:
                                if count_emoji == 1:
    ...
    async def rpnickcmd(self, message):
        ...
        if len(lst) > 3:
            ...
            if len(lst) + len(nick) >= 45:
                await message.edit(f"<b>Ник превышает лимит в 45 символов(возможно эмодзи имеют длину более 1 символа).</b>")
            else:
                    ...
                    if not dict == type(emj):
                        ...
                        for x in emj.values():
                            ...
                            if not x or not x.strip():
                                ...
                                return
                            if x.isalpha() or x.isspace() or x.isdigit() or x in string.punctuation:
                                await message.edit(f"<b>Были введены не только эмодзи в словаре для эмодзи(пробел тоже символ). </b>")
                                return
                            if len(lst) > 3:
                        ...
                        for x in emj:
                            ...
                            if int(len(key_len)) > 1:
                                await message.edit('<b>В качестве ключа было введено больше одного слова в словаре эмодзи.</b>')
                                return
                            if not x or not x.strip():
                ...
                if dict == type(com):
                    ...
                    for x in com:
                        ...
                        if int(len(key_len_cmd)) > 1:
                            await message.edit('<b>В качестве ключа было введено больше одного слова в словаре команд.</b>')
                            return
                        if not x or not x.strip():
                            await message.edit(f"<b>Пустой ключ в словаре для команд? Мой хороший, так дела не делаются.</b>")
                            return
                    for x in com.values():
                        if not x or not x.strip():
                    ...
                    if count_emoji == 1:
                        ...
                        await message.edit(f"<b>Успешное обновлени словаря команд и эмодзи! Вы можете просмотреть их благодаря команде '<code>.rplist</code>'.</b>")
                    else:
                        comand_copy = comand.copy()
                        merge_com = { **comand_copy, **com}
                        self.db.set('RPMod', 'rpcomands', merge_com)
                        await message.edit(f"<b>Успешное обновлени словаря команд! Вы можете просмотреть его благодаря команде '<code>.rplist</code>'.</b>")
                else:
                        ...
                        for i in comand:
                            if detail[0] == i:
                                if detail[0] in emojies.keys():
                                    if len(lines) < 2:
                                        if rezjim == 1:
                                            await message.edit(f"{emojies[detail[0]]} | <a href=tg://user?id={me.id}>{nick}</a> {comand[detail[0]]} <a href=tg://user?id={user.id}>{user.first_name}</a>"+detail[1])
                                        else:
                                        ...
                                        if rezjim == 1:
                                            await message.edit(f"{emojies[detail[0]]} | <a href=tg://user?id={me.id}>{nick}</a> {comand[detail[0]]} <a href=tg://user?id={user.id}>{user.first_name}</a>"+detail[1]+f"\n<b>С репликой: </b>{lines[1]}")
                                        else:
                                    ...
                                    if len(lines) < 2:
                                        if rezjim == 1:
                                            await message.edit(f"<a href=tg://user?id={me.id}>{nick}</a> {comand[detail[0]]} <a href=tg://user?id={user.id}>{user.first_name}</a>"+detail[1])
                                        else:
                                        ...
                                        if rezjim == 1:
                                            await message.edit(f"<a href=tg://user?id={me.id}>{nick}</a> {comand[detail[0]]} <a href=tg://user?id={user.id}>{user.first_name}</a>"+detail[1]+f"\n<b>С репликой: </b>{lines[1]}")
                                        else:
