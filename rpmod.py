...
class RPMod(loader.Module):
    ...
    async def watcher(self, message):
        ...
        if message.split(' ', 1)[0] not in self.rp:
            return
        ...
        entity = None
        try:
            entity = await self.client.get_entity(message.split(' ', 1))
        except:
            pass
        ...
        await utils.answer(message, f'<a href="tg://user?id={sender.id}">{sender.first_name}</a> {msg} <a href="tg://user?id={reply.id}">{reply.first_name}</a>')
